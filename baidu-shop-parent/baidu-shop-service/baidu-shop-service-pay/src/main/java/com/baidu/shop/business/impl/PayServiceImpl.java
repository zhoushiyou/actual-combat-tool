...
public class PayServiceImpl extends BaseApiService implements PayService {
    ...
    public void requestPay(PayInfoDTO payInfoDTO,  HttpServletResponse response) {
        ...
        if(orderInfoResult.getCode() == HTTPStatus.OK){
            OrderInfo orderInfo = orderInfoResult.getData();
            List<String> titleList = orderInfo.getOrderDetailList().stream()
                    .map(orderDetail -> orderDetail.getTitle()).collect(Collectors.toList());
            String titleStr = String.join(",", titleList);
            //如果商品标题大于10的话,就截取前10个字符,否则就展示全部
            ...
            //商品描述，可空
            String body = "Don't talk ！ Love me";
